        ...
        private FogTypePass SetMaterialUniforms(Camera camera)
        {
            ...
            if (sharedColorSettings)
            {
                ...
                if (activeSelectionType == ColorSelectionType.Gradient)
                    Material.SetTexture("_FogColorTexture0",
                        selectingFromDistance ? DistanceColorTexture : HeightColorTexture);
                else
                    Material.SetTexture("_FogColorTexture0",
                        selectingFromDistance ? distanceColorSource.colorRamp : heightColorSource.colorRamp);
            }
            else
            {
                if (distanceFog.enabled)
                    Material.SetTexture("_FogColorTexture0",
                        distanceFog.colorSelectionType == ColorSelectionType.Gradient
                            ? DistanceColorTexture
                            : distanceColorSource.colorRamp);

                if (heightFog.enabled)
                {
                    string colorTextureIdentifier =
                        fogType == FogTypePass.HeightOnly ? "_FogColorTexture0" : "_FogColorTexture1";
                    Material.SetTexture(colorTextureIdentifier,
                        heightFog.colorSelectionType == ColorSelectionType.Gradient
                            ? HeightColorTexture
                            : heightColorSource.colorRamp);
                }
            }
