Shader "RenderFeature/StylisticFog"
{
    Properties
    {
    ...
    inline float ComputeHeightFogAmount(float viewDirY, float effectiveDistance)
    {
        ...
        return _BaseDensity * (exp2(-relativeHeight * _DensityFalloff) - exp2(
            (-effectiveDistance * viewDirY - relativeHeight) * _DensityFalloff)) / viewDirY;
    }
    ...
    inline half4 BlendFogToScene(float2 uv, half4 fogColor, float fogAmount)
    {
        ...
        half4 blended = lerp(sceneColor, half4(fogColor.xyz, 1.),
                             fogColor.a * step(FOG_AMOUNT_CONTRIBUTION_THREASHOLD, fogAmount));
        blended.a = 1.;
        return blended;
    }
